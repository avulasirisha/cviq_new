...
App.controller('paymentInterviewerController', function ($scope, $http, $cookies,$state, $cookieStore, MY_CONSTANT, $timeout,ngDialog,responseCode, filterFilter) {
...
//==========================================================================================================================
    ...
    $scope.updatePercentage = function (data) {
        ...
        $http({
            method: 'GET',
            url: MY_CONSTANT.url_cviq + '/api/admin/getInterviewerPaymentDetail?interviewerID='+data.Id,
            headers:{
                'authorization':$cookieStore.get("obj").accessToken,
                'Content-type': 'application/x-www-form-urlencoded'
            }
            ...
            .success(function(response) {
                ...
                pendingList.forEach(function (column) {
                    ...
                    d.id = column._id;
                    d.interviewerID = column.interviewerID;
                    d.interviewerAmount = column.interviewerAmount;
                    ...
                    d.interviewStartDate = moment(column.interviewDate).format('MMM Do YYYY');//date
                    d.interviewStartTime = column.interviewStartTime;

                    d.interviewEndDate = moment(column.actualInterviewEndTime).format('MMM Do YYYY');//date
                    d.actualInterviewEndTime =moment(column.actualInterviewEndTime).format('h:mm a');//date
                ...
                $scope.myList = dataArray;

                console.log($scope.myList);
                for (var i = 0; i < $scope.myList.length; i++) {
                    $scope.myList[i].index = ( i )
                }
                $("#myModal").modal();
                // ngDialog.open({
                //     template: 'updateplan',
    ...
    $scope.updatePaymentStatus = function (data) {
        ...
        $http({
            method:'PUT',
            url: MY_CONSTANT.url_cviq +'/api/admin/updateInterviewerPaymentStatus',
            headers:{
                'authorization':$cookieStore.get("obj").accessToken,
                'Content-type': 'application/x-www-form-urlencoded'
            },
