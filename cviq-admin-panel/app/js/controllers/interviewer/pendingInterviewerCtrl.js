...
App.controller('pendingInterviewerController', function ($scope, $http, $cookies,$state, $cookieStore, MY_CONSTANT, $timeout,ngDialog,responseCode, filterFilter) {


    console.log("pending interviewer ctrl");
    'use strict';
...
//==========================================================================================================================
    $scope.getInterviewers = function(skip,limit){
        ...
        $http({
            method: 'GET',
            url: MY_CONSTANT.url_cviq + '/api/admin/getPendingInterviewersList',
            headers:{
            ...
            .success(function(response){
                ...
                var dataArray = [];
                var pendingList = response.data.pendingRequests;
    ...
    $scope.exportData = function(){
        $http({
            ...
            .success(function(response){
                ...
                var dataArray1 = [];
                var index1 = 0;
                var pendingList1 = response.data.pendingRequests;
                ...
                alasql('SELECT * INTO CSV("PendingInterviewersList.csv",{headers:true}) FROM ?',[$scope.exportList]);
            })
...
//==========================================================================================================================
    ...
    $scope.approveInterviewer = function(id,name){
console.log(id);
       $scope.interviewer_name = name;
       $scope.interviewer_id = id;
        ngDialog.open({
            template: 'approve_interviewer_modalDialog',
            className: 'ngdialog-theme-default',
            showClose: false,
            scope: $scope
        });
    };
    $scope.confirmApproveInterviewer = function(id){
        $('.myButton').prop('disabled', true);
        console.log(id);

        $http({
            method: 'PUT',
            url: MY_CONSTANT.url_cviq + '/api/admin/verifyInterviewerProfile',
            headers:{
                'authorization':$cookieStore.get("obj").accessToken,
                'Content-type': 'application/x-www-form-urlencoded'
            },
