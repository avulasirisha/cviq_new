App.controller('CandidatememController', function ($state,$scope, $modal, $http, $cookies, $cookieStore, MY_CONSTANT, $timeout, ngDialog, responseCode, filterFilter) {
    ...
    $http({
        method:'GET',
        url: MY_CONSTANT.url_cviq + '/api/common/getCandidateMemberships',
    })
        .success(function (response) {
            ...
            pendingList.forEach(function (column) {
                var d = {};
                d.Id = column._id;
                d.planType = column.planType;
                d.planRate = column.planRate;
                d.planDuration = column.planDuration;
                d.planDescription = column.planDescription;
                d.isDeleted   = column.isDeleted;
                d.status = column.isDeleted == true ? "deleted"  : "active";
                dataArray.push(d);
            });
...
//==========================================================================================================================
    $scope.exportData = function () {
        ...
        $http({
            ...
            .success(function (response) {
                console.log(response);
                var exportArray = [];
                var pendingList1 = response.data;
                ...
                alasql('SELECT * INTO CSV("CandidateMembership.csv",{headers:true}) FROM ?', [$scope.exportList]);
            })
    ...
    $scope.updateCountry = function (data) {
    ...
    $scope.updateConfirm = function () {
      ...
      //  if()
      $scope.newdata.memID =$scope.newdata.Id;
      delete $scope.newdata.Id;
      delete $scope.newdata.index;
      delete $scope.newdata.status;      
        $http({
            method:'PUT',
            url: MY_CONSTANT.url_cviq +'/api/admin/updatecandidateMembership',
            headers:{
                'authorization':$cookieStore.get("obj").accessToken,
                'Content-type': 'application/x-www-form-urlencoded'
            },
 ...
 $scope.addMembership = function () {
    ...
    $scope.addConfirm = function () {
        ...
        $http({
            method:'PUT',
            url: MY_CONSTANT.url_cviq +'/api/admin/addCandidateMembership',
            headers:{
                'authorization':$cookieStore.get("obj").accessToken,
                'Content-type': 'application/x-www-form-urlencoded'
            },
            data:$scope.addnewData
        })
            .success(function(response){
                $scope.addnewData = {};
                console.log(response);
