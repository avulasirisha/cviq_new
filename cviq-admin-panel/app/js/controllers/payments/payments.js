...
App.controller('paymentController', function ($scope, $http, $cookies,$state, $cookieStore, MY_CONSTANT, $timeout,ngDialog,responseCode, filterFilter) {
...
//==========================================================================================================================
    $scope.getcandidates = function(skip,limit){
        ...
        $http({
            method: 'GET',
            url: MY_CONSTANT.url_cviq + '/api/admin/getAllPaymentDetail',
            headers:{
            ...
            .success(function(response){
                ...
                var dataArray = [];
                var allList = response.data.details;
                ...
                allList.forEach(function (column) {
                    ...
                    d.paymentId = column.paymentId;
                    d.createdAt = moment( column.createdAt ).format('MMM Do YYYY, h:mm a'); 
                    d.promocode = column.promocode;
                    d.userId    = column.userId;
                    d.username = column.username;
                    dataArray.push(d);
    ...
    $scope.exportData = function(){
        $http({
            ...
            .success(function(response){
                ...
                var dataArray1 = [];
                var allList1 = response.data.details;
                ...
                allList1.forEach(function (column) {
                    ...
                    e.amount = column.amount;
                    e.paymentId = column.paymentId;
                    e.createdAt = moment( column.createdAt ).format('MMM Do YYYY, h:mm a'); 
                    e.promocode = column.promocode;
                    e.userId    = column.userId;
                    e.username = column.username;
                    dataArray1.push(e);
                });
                $scope.exportList = dataArray1;
                //console.log($scope.exportList);
                alasql('SELECT * INTO CSV("AllPaymentsList.csv",{headers:true}) FROM ?',[$scope.exportList]);
            })
