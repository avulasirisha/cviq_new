angular.module('Cviq').controller('appliedJobsCtrl', ['$scope','$rootScope','ngDialog','$http','CONSTANT','characterService','$state','$cookieStore','$timeout','$window', function($scope, $rootScope, ngDialog, $http, CONSTANT, characterService, $state, $cookieStore, $timeout, $window){
...
var D_data = JSON.parse(localStorage.getItem('UserDetails'));
    ...
    $scope.pageFunction = function (data) {
        ...
        if(data && data > 1) {
            ...
            $http({
            method: 'GET',
            url: CONSTANT.apiUrl + '/api/candidate/candidateAppliedJobs',
            headers: {
